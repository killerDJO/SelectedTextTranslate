...
void ConfirmDialogContentControl::SetDimensions(Point position, int width)
{
    ...
    ViewState->SetLayout(LayoutDescriptor::CreateFixedLayoutDescriptor(position, Size(width, height)));
}
...
void ConfirmDialogContentControl::RenderContent(Renderer* renderer)
{
    ...
    renderer->DrawBorderedRect(Rect(Point(0, 0), ViewState->GetViewSize()), nullptr, borderWidth, Colors::Gray);
    ...
    renderer->DrawRect(Rect(renderPosition.GetPosition(), Size(ViewState->GetViewSize().GetWidth(), borderWidth)), grayBrush);
    ...
    renderer->DrawBorderedRect(
        Rect(renderPosition.GetPosition(), Size(ViewState->GetViewSize().GetWidth(), ViewState->GetViewSize().GetHeight() - renderPosition.GetY())),
        backgroundBrush,
        borderWidth,
        Colors::Gray);
    ...
    confirmButton->SetPosition(Point(
        ViewState->GetViewSize().GetWidth() - paddingX - confirmButton->GetComputedSize().GetWidth(),
